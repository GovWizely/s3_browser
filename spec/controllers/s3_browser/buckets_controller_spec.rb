...
module S3Browser
  RSpec.describe BucketsController, type: :controller do
    ...
    describe "GET index" do
      before  { get :index, params: { bucket_name: bucket_name }}
    ...
    describe "GET upload" do
      context 'when one or more files are selected' do
        before { get :upload, params: { bucket_name: bucket_name, files: ['array', 'of', 'files'] }}
      ...
      context 'when no file is selected' do
        before { get :upload, params: { bucket_name: bucket_name, files: nil }}
    ...
    describe "GET delete" do
      context 'when a file is selected for deletion' do 
        before { get :delete, params: { bucket_name: bucket_name, filename_to_delete: 'foo_file' }}
        it "redirects to the index template" do
          expect(response).to redirect_to(:action => "index", :bucket_name => 'foo-bucket')
        end
      end
      ...
      context 'when no file is selected' do
        before { get :delete, params: { bucket_name: bucket_name, filename_to_delete: nil }}
        it "rerenders the form with an error message" do
          expect(response).to render_template(:index)
    ...
    describe '#bucket' do 
      before  do
        ...
        get :index, params: { bucket_name: bucket_name }
      end
